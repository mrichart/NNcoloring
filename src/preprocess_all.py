...
patch_sizes = [ [5,5], [11,11] ]      #to be generated, sizes must be odd
directoryData = 'dataset/Opencountry1'         #raw data
som_sizes = [[3,3], [5,5],[10,10]]  #soms to use
...
for som_size in som_sizes:
    ...
    for patch_size in patch_sizes:
        ...
        fileData = open ('pkls/data_all_patch' + str(patch_size[0]) + 'x' + str (patch_size[1])+'_som' + str(som_size[0]) + 'x' + str (som_size[1])+'.pkl','wb') 
        pickle.dump (X, fileData)
        fileData.close()

        fileLabels = open ('pkls/labels_all_patch' + str(patch_size[0]) + 'x' + str (patch_size[1])+'_som' + str(som_size[0]) + 'x' + str (som_size[1])+'.pkl','wb') 
